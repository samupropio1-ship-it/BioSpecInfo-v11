...
| **Documented version** | Service Worker `bsi-v171` |
...
1. **Grounding** — a fabricated chemical value can be dangerous. 35 tools cover
...
│        │      · Gemini                                              │
│        │                                                            │
│        └── tool executor (35 tools, all local except 2 network)     │
...
prompt and the schema of all 35 tools → receive the streamed response → if it
...
The list is the one **measured** in `window.BSI_AI_TOOLS` on the running
application, not transcribed from memory: this document declared 32 when there
were already 35, and three — `analizza_molecola`, `disegna_molecola`,
`mostra_spettri` — did not appear at all.
...
| **Structure & spectra** | `spettroscopia`, `quantistica_e_spettroscopia`, `cristallografia`, `analizza_molecola`, `disegna_molecola`, `mostra_spettri` |
...
> **Web search is not among these thirty-five**, and the distinction matters.
> The thirty-five are executed **by the application**, in the user's browser. On
> providers that offer them, the agent adds two tools to the schema that are
> executed **by the provider** — `web_search` and `web_fetch`, with a per-turn
> usage cap. Counting them with the others would make them look always
> available, whereas they depend on the provider chosen.
...
Drugs and strategies are the two figures that `tools/verifica-affermazioni.js`
measures against the running application on every battery run: here they had
been left at 143 and 39 while the app counted 178 and 46.
